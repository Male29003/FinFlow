...
namespace WebAPIs.Controllers.v1
{
    ...
    public class AuthController : BaseApiController
    {
        ...
        /// <summary>
        /// Bước 1: Gửi OTP (Có thể dùng cho Đăng ký, Quên MK)
        /// Yêu cầu body: { "email": "...", "purpose": "ForgotPassword" }
        /// </summary>
        [HttpPost("send-otp")]
        [AllowAnonymous]
        public async Task<IActionResult> SendOtp([FromBody] AuthFeatures.SendOtp command)
        {
            return Ok(await Mediator.Send(command));
        }

        /// <summary>
        /// Bước 2: Xác thực OTP độc lập (Cho phép Frontend nhảy từ Step 2 sang Step 3)
        /// Yêu cầu body: { "email": "...", "otpCode": "123456", "purpose": "ForgotPassword" }
        /// </summary>
        [HttpPost("verify-otp")]
        [AllowAnonymous]
        public async Task<IActionResult> VerifyOtp([FromBody] AuthFeatures.VerifyOtp command)
        {
        ...
        /// <summary>
        /// Bước 3: Xác thực lại lần cuối và Lưu mật khẩu mới
        /// Yêu cầu body: { "email": "...", "otpCode": "...", "newPassword": "..." }
        /// </summary>
        [HttpPost("reset-password")]
        [AllowAnonymous]
        public async Task<IActionResult> ResetPasswordWithOtp([FromBody] AuthFeatures.ResetPasswordWithOtp command)
        {
            var result = await Mediator.Send(command);
            if (result.Code != 200) return BadRequest(new { message = result.Message });
            return Ok(result);
